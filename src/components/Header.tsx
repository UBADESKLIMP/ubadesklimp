...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${
      isScrolled 
        ? isAutomotivoPage 
          ? 'bg-[#0c1424]/90 backdrop-blur-lg shadow-md border-b border-blue-900/30'
          : 'bg-background/98 backdrop-blur-lg shadow-md border-b border-border/80'
        : isAutomotivoPage
          ? 'bg-[#0c1424]/85 backdrop-blur-md shadow-soft border-b border-blue-900/20'
              ...
              user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className={`${isAutomotivoPage ? 'text-white hover:text-blue-400' : 'text-foreground hover:text-primary'}`}>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${
        ...
        <div className={`px-4 pt-3 pb-4 space-y-1 ${isAutomotivoPage ? 'bg-[#0c1424]/90 border-blue-900/30' : 'bg-background/98 border-border/50'} backdrop-blur-sm border-t shadow-lg`}>
